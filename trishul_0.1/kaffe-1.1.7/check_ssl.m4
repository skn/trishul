...
AC_ARG_WITH([ssl],
            AS_HELP_STRING([--with-ssl=SSL],
                           [Use the specified OpenSSL installation]),
[
	openssl_search_dir="$withval"
],
[
	openssl_search_dir="/usr/local/ssl /usr/lib/ssl /usr/ssl /usr/pkg /usr/local /usr"
])

    for dir in $openssl_search_dir; do
        ssldir="$dir"
