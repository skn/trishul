...
Java_java_lang_reflect_Method_invoke0(JNIEnv* env, jobject _this, jobject _obj, jobjectArray _argobj)
{
	...
	taint_t objTaint = nativeParamGetTaint (1); /* PS. */
	...
	for (i = len - 1; i >= 0; i--) {
		...
		args[i].taint = getArrayElementTaint ((Hjava_lang_Object *) argobj, i); /* PS */
...
#define CallTypeMethodA(type) \
	(*env)->Call##type##MethodTaintA(env, obj, objTaint, methID, args)
...
#undef CallTypeMethodA
	...
	{
		ret.taint = nativeRetValGetTaint (rettype);
		switch (rettype) {
		case 'I':
			ret.l = ((*env)->NewObjectA(env, Iclass, Imeth, &ret));
			break;
		case 'Z':
			ret.l = ((*env)->NewObjectA(env, Zclass, Zmeth, &ret));
			break;
		case 'S':
			ret.l = ((*env)->NewObjectA(env, Sclass, Smeth, &ret));
			break;
		case 'B':
			ret.l = ((*env)->NewObjectA(env, Bclass, Bmeth, &ret));
			break;
		case 'C':
			ret.l = ((*env)->NewObjectA(env, Cclass, Cmeth, &ret));
			break;
		case 'F':
			ret.l = ((*env)->NewObjectA(env, Fclass, Fmeth, &ret));
			break;
		case 'D':
			ret.l = ((*env)->NewObjectA(env, Dclass, Dmeth, &ret));
			break;
		case 'J':
			ret.l = ((*env)->NewObjectA(env, Jclass, Jmeth, &ret));
			break;
		case 'L':
		case '[':
			break;
		...
		nativeRetValSetTaint (rettype, ret.taint);
		return ret.l;
	}
