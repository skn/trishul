			      ...
			      jint len)
{
	...
	taintLen = len * sizeof (taint_t);
	len *= elemsz;
	srcpos *= elemsz;
	dstpos *= elemsz;

	in = &((char*)ARRAY_DATA(src))[srcpos];
	inTaint = src->member_taint;
	out = &((char*)ARRAY_DATA(dst))[dstpos];
	outTaint = dst->member_taint;

	if (sclass == dclass) {
#if defined(HAVE_MEMMOVE)
		memmove((void*)out, (void*)in, (size_t)len);
		memmove((void*)outTaint, (void*)inTaint, (size_t)taintLen);
#else
		/* Do it ourself */
#if defined(HAVE_MEMCPY)
		if (src != dst) {
			memcpy((void*)out, (void*)in, (size_t)len);
			memcpy((void*)outTaint, (void*)inTaint, (size_t)taintLen);
		} else
#endif
		if (out < in) {
			/* Copy forwards */
