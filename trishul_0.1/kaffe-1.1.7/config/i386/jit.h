...
#define	REGISTER_SET							\
	{ /* eax */	NULL, 0, Rint|Rsubint|Rref,0, 0, 0, 0 },		\
	{ /* ecx */	NULL, 0, Rint|Rsubint|Rref,0, 0, 1, 0 },		\
	{ /* edx */	NULL, 0, Rint|Rsubint|Rref,0, 0, 2, 0 },		\
	{ /* ebx */	NULL, 0, Rint|Rsubint|Rref,Rglobal|Rnosaveoncall, 0, 3, 0 },\
	{ /* esp */	NULL, 0, Reserved,		0, 0, 4, 0   },		\
	{ /* ebp */	NULL, 0, Reserved,		0, 0, 5, 0   },		\
	{ /* esi */	NULL, 0, Rint|Rref,	Rglobal|Rnosaveoncall, 0, 6, 0 },\
	{ /* edi */	NULL, 0, Rint|Rref,	Rglobal|Rnosaveoncall, 0, 7, 0 },\
	{ /* f0  */	NULL, 0, Rfloat|Rdouble,	Rreadonce, 0, 8, 0     },
...
#define LOCAL_SLOT_COUNT 	(maxLocal + maxStack + maxTemp - maxArgs)
#define CONTEXT_PART_SIZE 	((globalMethod->context_count + 1) * sizeof (taint_t))
#define FRAMESIZE			(SLOTSIZE + sizeof (taint_t)) * (LOCAL_SLOT_COUNT) + CONTEXT_PART_SIZE
...
#define SLOT2ARGOFFSET_TAINT(_n)	(8 + SLOTSIZE * (maxArgs + 1) + sizeof (taint_t) * (_n))


/* Generate slot offset for a local (non-argument) */
#if defined(JIT3)
#define	SLOT2LOCALOFFSET(N)			((-(SLOTSIZE + sizeof (taint_t)) * ((N) - maxArgs + 1) + 4) - CONTEXT_PART_SIZE)
#define	SLOT2LOCALOFFSET_TAINT(N)	((-(SLOTSIZE + sizeof (taint_t)) * ((N) - maxArgs + 1)) - CONTEXT_PART_SIZE)
#else
