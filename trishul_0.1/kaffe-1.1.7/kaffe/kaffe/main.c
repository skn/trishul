...
#include "polymer.h"
// In TrishulSignatures.c
extern const NativeLibraryDigests NATIVE_LIBRARYS_DIGESTS[];
extern const char *SECURE_CLASSPATH;
extern const char *TRISHUL_DEFAULT_ENGINE;
extern const char *TRISHUL_DEFAULT_ENGINE_POLICY;
extern unsigned int TRISHUL_CERT_LENGTH;
extern const unsigned char TRISHUL_CERT[];
extern const char *TRISHUL_SIGNATURE_ALIAS;

static const char 	*engineClass;
static const char 	*enginePolicyURL;

#ifdef KAFFE_VMDEBUG
/* Receives the classname specified on the command line, with periods translated into slashes.
 * This is required when context debug methods are used, to speed up name comparisons.
 */
static char			mainClassNameSlashes[1024];
static char 		*contextDebugMethods;
static char 		*taintDebugClasses;
#endif


#ifdef TRISHUL_MEASURE_CODE_SIZE
extern int code_generated;
#endif

#ifdef TRISHUL_MEASURE_FALLBACK
extern uint context_failed_count;
extern uint context_tested_count;
#endif

#ifdef TRISHUL_MEASURE_ANALYSIS_SIZE
extern uint recordAllocContextPolicy;
extern uint recordAllocStorageRecord;
#endif
...
static int checkException2(JNIEnv* env);
static int main2(JNIEnv* env, char *argv[], int farg, int argc);
static userProperty*setUserProperty(char * propStr);
...
const uint8 *jarVerifyGetCertificateCallback (const char *name, SignatureType type, uint *certLength)
{
	if (!strcasecmp (name, "TRISHUL"))
	{
		assert (type == signatureTypeDSA);
		*certLength = TRISHUL_CERT_LENGTH;
		return TRISHUL_CERT;
	}
	return NULL;
}
...
main(int argc, char* argv[])
{
	...
	trishulSetNativeLibraryDigests (NATIVE_LIBRARYS_DIGESTS); // PS
	setJarVerifyCallback (jarVerifyGetCertificateCallback);
	if (TRISHUL_DEFAULT_ENGINE[0])
		engineClass = strdup (TRISHUL_DEFAULT_ENGINE);
	if (TRISHUL_DEFAULT_ENGINE_POLICY[0])
		enginePolicyURL = strdup (TRISHUL_DEFAULT_ENGINE_POLICY);
	...
	/* PS */
	/* make sure any allocated data is released */
	atexit (taintRelease);

#ifdef POLYMER_ENGINE_USE_PROTECTION_DOMAIN
	/* If protection domains are used, a security manager must be enabled */
	/* This is done here, so that a security manager specified on the command line overrides this */
	setUserProperty (strdup ("java.security.manager"));
#endif
	/* PS- */
	...
	/* PS */
#ifdef KAFFE_VMDEBUG
	/* Set context debug methods.
	 * This must be done before the main class is loaded, as that will already trigger context analysis
	 */
	if (contextDebugMethods)
	{
		char *methodName = strtok (contextDebugMethods, ",");
		while (methodName)
		{
			char *lastDot = strrchr (methodName, '.');
			if (!lastDot)
			{
				if (mainClassNameSlashes[0] == 0)
				{
					char *ptr;
					strncpy (mainClassNameSlashes, argv[farg],
							 sizeof (mainClassNameSlashes) / sizeof (mainClassNameSlashes[0]));
					for (ptr = mainClassNameSlashes; *ptr; ptr++)
						if (*ptr == '.')
							*ptr = '/';
				}

				trishulAddContextDebugMethod (mainClassNameSlashes, methodName);
			}
			else
			{
				*lastDot = 0;
				trishulAddContextDebugMethod (methodName, lastDot + 1);
			}

			methodName = strtok (NULL, ",");
		}
	}
#endif /* KAFFE_VMDEBUG */

	vmargs.secureClasspath = SECURE_CLASSPATH;
	userProperty * prop;
    prop = malloc(sizeof(userProperty));
    prop->next 		= userProperties;
	prop->key 		= strdup ("trishul.secure.classpath");
	prop->value 	= strdup (vmargs.secureClasspath);
    userProperties 	= prop;
/* PS- */
...
main2(JNIEnv* env, char *argv[], int farg, int argc)
{
	...
	if (checkException2(env))
		goto exception_happened;

	/* PS */
#ifdef KAFFE_VMDEBUG
	taintInitialize (mcls);

	/* Add extra debug classes */
	if (taintDebugClasses)
	{
		char *className = strtok (taintDebugClasses, ",");
		while (className)
		{
			jclass dcls = (*env)->FindClass (env, className);
			if (checkException2(env))
				goto exception_happened;
			addTaintDebugClass (dcls);
			if (checkException2(env))
				goto exception_happened;
	...
	/* Load the engine if one is specified */
	if (engineClass)
	{

		char *className = strtok ((char *) engineClass, ",");
		while (className)
		{
			...
			addEngineClass (cls, enginePolicyURL);
			if (checkException2(env))
				goto exception_happened;
...
#ifdef TRISHUL_MEASURE_CODE_SIZE
	fprintf (stderr, "Code size: %u bytes\n", code_generated);
#endif

#ifdef TRISHUL_MEASURE_FALLBACK
	fprintf (stderr, "Context: %u failed out of %u\n", 	context_failed_count, context_tested_count);
#endif

#ifdef TRISHUL_MEASURE_ANALYSIS_SIZE
	fprintf (stderr, "Context size: %u bytes: %u context, %u storage\n", recordAllocContextPolicy + recordAllocStorageRecord,
		recordAllocContextPolicy, recordAllocStorageRecord);
#endif
...
checkException2(JNIEnv* env)
{
	...
	if ((*env)->IsInstanceOf(env, e, eiic))
	{
		...
		Method *meth = (*env)->GetMethodID(env, exClass, "getException", "()Ljava/lang/Throwable;");
		e = (*env)->CallObjectMethod(env, e, meth);
		if ((*env)->ExceptionOccurred(env) != NULL) {
			(*env)->ExceptionClear(env);
			return (1);
		}
...
options(char** argv, int argc)
{
	...
	for (i = 1; i < argc; i++) {
		...
		/* PS */
		if (!TRISHUL_DEFAULT_ENGINE[0] && strcmp(argv[i], "-Xengine") == 0) {
			i++;
			if (argv[i] == 0) {
				fprintf(stderr,
						"Error: No engine class found for %s option.\n",
						argv[i - 1]);
				exit(EXIT_FAILURE);
			}
			engineClass = argv[i];
		}
		else if (!TRISHUL_DEFAULT_ENGINE_POLICY[0] && strcmp(argv[i], "-XenginePolicy") == 0) {
			i++;
			if (argv[i] == 0) {
				fprintf(stderr,
						"Error: No policy file found for %s option.\n",
						argv[i - 1]);
				exit(EXIT_FAILURE);
			}
			enginePolicyURL = argv[i];
		}
#ifdef KAFFE_VMDEBUG
		else if (strcmp(argv[i], "-Xtaintdebuglevel") == 0) {
			i++;
			if (argv[i] == 0) {
				fprintf(stderr,
				    "Error: No level found for %s option.\n",
				    argv[i - 1]);
				exit(EXIT_FAILURE);
			}

			char *endptr;
			int level = strtol (argv[i], &endptr, 10);
			if (*endptr)
			{
				fprintf(stderr,
				    "Error: Invalid level found for %s option.\n",
				    argv[i - 1]);
				exit(EXIT_FAILURE);
			}
			setTaintDebugLevel (level);
		}
		else if (strcmp(argv[i], "-Xtaintdebugclasses") == 0) {
			i++;
			if (argv[i] == 0) {
				fprintf(stderr,
						"Error: No debug classes found for %s option.\n",
						argv[i - 1]);
				exit(EXIT_FAILURE);
			}

			taintDebugClasses = argv[i];
		}
		else if (strcmp(argv[i], "-Xcontextdebugmethods") == 0) {
			i++;
			if (argv[i] == 0) {
				fprintf(stderr,
						"Error: No debug methods found for %s option.\n",
						argv[i - 1]);
				exit(EXIT_FAILURE);
			}

			contextDebugMethods = argv[i];
		}
		else if (strcmp(argv[i], "-Xcontextdebugflow") == 0) {
			trishulSetContextDebugMethodFlow (true);
		}
#endif /* KAFFE_VMDEBUG */
		else
		/* -PS */
...
usage(void)
{
	...
	fprintf(stderr, _("\n"
			  "Trishul options:\n"
			  "	-Xegnine <class>		 Engine main class\n"
			  "	-XenginePolicy <url>		 Engine's policy\n"
#ifdef KAFFE_VMDEBUG
			  "	-Xtaintdebuglevel <int>  	Level of enabled taint debug messages\n"
			  "	-Xtaintdebugclasses <list> 	List of comma-separated classes that generate taint debug messages\n"
			  "	-Xcontextdebugmethods <list>	List of comma-separated classes that generate context debug messages\n"
#endif /* KAFFE_VMDEBUG */
			  ));
}
