...
#include "md.h"
#include "gc.h"
#include "context-analysis.h" /* PS */
#include "decompile.h" /* PS */
#include "utils.h" /* PS */
#include "stats.h" /* PS */

#ifdef TRISHUL_MEASURE_FALLBACK
uint context_failed_count = 0;
uint context_tested_count = 0;
#endif
...
#ifdef TRISHUL_MEASURE_ANALYSIS_TIME
	static timespent spent;
	startTiming(&spent, "analysis");
#endif
...
DBG(CODEANALYSE,
	...
	/* Do this before contextAnalysisBeginAnalyze, so the catch types are resolved */
	...
	/* PS */
	struct ContextAnalysisInfo *polInfo;
	polInfo = contextAnalysisBeginAnalyze (meth);
	...
	for (pc = 0; pc < codeInfo->codelen;) {
		...
		case IFNULL:	case IFNONNULL:
			tabpc = pc + WORD(pc+1);
			contextAnalysisControlFlowInstruction (polInfo, pc, tabpc, INSNLEN(pc),
												 tabpc > pc ? BBT_IF_FORWARD : BBT_IF_BACKWARD); /* PS */
		...
		case GOTO:
			tabpc = pc + WORD(pc+1);
			contextAnalysisControlFlowInstruction (polInfo, pc, tabpc, INSNLEN(pc),
												 tabpc > pc ? BBT_GOTO_FORWARD : BBT_GOTO_BACKWARD); /* PS */
			SET_STARTOFBASICBLOCK(tabpc);
			SET_JUMPFLOW(pc, tabpc);
			pc = pc + INSNLEN(pc);
		...
		case TABLESWITCH:
			...
			switchBlock = contextAnalysisControlFlowInstruction (polInfo, pc, idx,
							instruction_length (meth->c.bcode.code + pc, meth->c.bcode.code + meth->c.bcode.codelen),
							BBT_TABLESWITCH); /* PS */

			for (; idx > 0; idx--) {
				SET_STARTOFBASICBLOCK(pc+DWORD(tabpc+idx*4+8));
				SET_JUMPFLOW(pc, pc+DWORD(tabpc+idx*4+8));
				contextAnalysisSwitchCase (polInfo, switchBlock, idx - 1, pc, pc+DWORD(tabpc+idx*4+8)); /* PS */
			}
			SET_STARTOFBASICBLOCK(pc+DWORD(tabpc));
			SET_JUMPFLOW(pc, pc+DWORD(tabpc));
			contextAnalysisSwitchCase (polInfo, switchBlock, -1, pc, pc+DWORD(tabpc)); /* PS */
		...
		case LOOKUPSWITCH:
			...
			for (; idx > 0; idx--) {
				...
				contextAnalysisSwitchCase (polInfo, switchBlock, idx - 1, pc, pc+DWORD(tabpc+idx*8+4)); /* PS */
			}
			SET_STARTOFBASICBLOCK(pc+DWORD(tabpc));
			SET_JUMPFLOW(pc, pc+DWORD(tabpc));
			contextAnalysisSwitchCase (polInfo, switchBlock, -1, pc, pc+DWORD(tabpc)); /* PS */
		...
		case ATHROW:	case RET:
			contextAnalysisControlFlowInstruction (polInfo, pc, -1, INSNLEN(pc),
												 INSN(pc) == ATHROW ? BBT_THROW : BBT_RETURN); /* PS */
			pc = pc + INSNLEN(pc);
			if (pc < codeInfo->codelen) {
				SET_STARTOFBASICBLOCK(pc);
			}
		...
		case INVOKEINTERFACE:
			// An error will be caught during analysis later
			if (getMethodSignatureClass(WORD(pc+1), meth->class, true,
					(INSN(pc) == INVOKESPECIAL ? 1 : ((INSN(pc) == INVOKEINTERFACE) ? 2 : 0)), &call, einfo) &&
				call.method->ndeclared_exceptions)
			{
				contextAnalysisMethodCallException (polInfo, pc, INSNLEN(pc), call.method);
			}
	...
	einfo->type = 0;
	contextAnalysisEndAnalyze (einfo, polInfo, meth, pc);
	if (einfo->type)
		return false;
