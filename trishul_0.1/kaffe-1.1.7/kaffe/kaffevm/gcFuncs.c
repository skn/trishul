...
walkMethods(Collector* collector, void *gc_info, Method* m, int nm)
{
		...
		if (objIndex == KGC_ALLOC_JITCODE
		  ...
		  KGC_markObject(collector, gc_info, m->c.bcode.code);
			...
			/* PS: if a hook is set, make sure the reflection object is kept alive */
			if (m->hook_reflected)
			{
				KGC_markObject(collector, gc_info, m->hook_reflected);
			}
...
DBG(GCPRECISE,
	...
	/* PS: mark the taint data for the elements */
	KGC_markObject(collector, gc_info, arr->member_taint);
...
static
void
walkPrimArray(Collector* collector, void *gc_info, void* base, uint32 size UNUSED)
{
        Hjava_lang_Object* arr;
	iLock *lk;
...
DBG(GCPRECISE,
		...
		/* PS: mark the taint data for the members */
		KGC_markObject(collector, gc_info, obj->member_taint);
...
initCollector(void)
{
	...
	KGC_registerGcTypeByIndex(gc, KGC_ALLOC_PRIMARRAY,
	    walkPrimArray, finalizeObject, NULL, "prim-arrays");
