		...
		struct Hjava_lang_Object* p, int nullOK)
{
	...
	do
	  {
		...
		*/
		...
		if (p != NULL || !nullOK) {
		  do_execute_java_method(NULL, p, 0, "println",
					 "([C)V", NULL, 0, str);
		} else {
		  dprintf("%s\n", buf);
		}
		...
		if (unhand(nextCause)->detailMessage != NULL)
		  {
		    char *cstr = checkPtr (stringJava2C(unhand(nextCause)->detailMessage));
		...
		if (p != NULL || !nullOK)
		  {
		    do_execute_java_method(NULL, p, 0, "println", "([C)V", NULL, 0, str);
		  }
		else
		  {
		    dprintf("%s\n", buf);
	...
	if (p != NULL || !nullOK) {
	  do_execute_java_method(NULL, p, 0, "flush", "()V", NULL, 0);
	}
}
