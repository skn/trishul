...
initialiseSecurity (void)
{
	...
	do_execute_java_method(NULL, permissions, 0, "add", "(Ljava/security/Permission;)V",
			       NULL, 0, allPermission);
	do_execute_java_method(NULL, permissions, 0, "setReadOnly", "()V", NULL, 0);
...
/* PS
 * Adds superclass->total_instance_field_count to the field indices and calculates class->total_instance_field_count.
 * If required, superclass is also processed.
 */
static void patch_field_indices (Hjava_lang_Class* class)
{
	/* Process the superclass, if required */
	if (class->superclass && class->superclass->total_instance_field_count < 0)
	{
		patch_field_indices (class->superclass);
	}

	/* Iterate over the fields only if there is a point to it */
	if (class->superclass && class->superclass->total_instance_field_count > 0)
	{
		int i;

		/* The static fields don't need to be processed */
		for (i = CLASS_NSFIELDS (class); i < CLASS_NFIELDS (class); i++)
		{
			CLASS_FIELDS(class)[i].trishul.taint_index += class->superclass->total_instance_field_count;
		}

		class->total_instance_field_count = class->superclass->total_instance_field_count + CLASS_NIFIELDS(class);
	}
	else
	{
		class->total_instance_field_count = CLASS_NIFIELDS(class);
	}
}
...
DBG(STATICINIT, dprintf("Initialising %s static %d\n", class->name->data,
		...
		KaffeVM_safeCallMethodA(meth, METHOD_NATIVECODE(meth), NULL, 0, NULL, NULL, 0);
	...
	// PS
	if (success &&
		class->state > CSTATE_DOING_SUPER &&
		class->total_instance_field_count < 0)
	{
		/* Adjust the field indices for superclasses */
		patch_field_indices (class);

		// Try to load the annotations class
		errorInfo errorAnnotations;
		Utf8Const *name = trishulAnnotationClassName (class);

		// Try to load the annotation class
		class->annotations = loadClass (name, NULL, &errorAnnotations);
		utf8ConstRelease (name);
		if (class->annotations)
		{
			// Found it, now process it
			trishulProcessAnnotations (class);
		}
	}
...
DBG(RESERROR,	dprintf("addField: no signature name.\n");		);
	...
	if (access_flags & ACC_STATIC) {
		CLASS_NSFIELDS(c)++;
		ft->trishul.taint = 0; // PS
	}
	else { // PS
		/* The superclass' index is added in processClass, when class-loading completes */
		ft->trishul.taint_index = (CLASS_NFIELDS(c) - CLASS_NSFIELDS(c)) - 1;
	}
...
finishFields(Hjava_lang_Class *cl)
{
	...
	// PS
//	cl->total_fields = (cl->superclass ? cl->superclass->total_fields : 0) + CLASS_NFIELDS(cl);
}
	...
	if( (jname = utf8Const2JavaReplace(ce->name, '/', '.')) )
	  {
	    ...
	    args[0].l = jname;
	    KaffeVM_safeCallMethodA(meth, METHOD_NATIVECODE(meth), loader, 0, args, &jretval, false);
	    retval = jretval.l;
