...
#include "errors.h"
#include "jthread.h"
#include "hashtab.h" // PS
#include "locks.h"

/* PS */
struct ContextBasicBlock;
struct StorageRecord;
typedef unsigned char RuntimeBitmapElement;
/* PS- */
...
struct Hjava_lang_Class {
	...
	/* PS
	 * Number of instance fields, including super classes' fields. This is required to form the index
	 * into the taint array, which must be after the superclass' fields.
	 * As the class loaded may load a class before the superclass is finalized, the fields' index initially does
	 * not take this into account. Only when the class is fully loaded, and thus the superclass too, is this
	 * added to every instance field of subclasses.
	 */
	short	total_instance_field_count;

	struct Hjava_lang_Class *annotations;
	// -PS
...
enum
{
	TRISHUL_NOTAINT_METHOD = 1,
	TRISHUL_NOTAINT_FALLBACK = 2,
};
...
#define declared_exceptions declared_exceptions_u.local_exceptions
	...
	Method						*hook;
	bool						hook_always;
	hashtab_t					hook_objects;
	void						*hook_reflected; /* Really Hjava_lang_reflect_Method or Constructor */
	uint						noTaintFlags;    /* Set if the method does not hold its taint,
																only accessible through annotations */

	struct ContextBasicBlock	*contexts;
	uint						context_count;
	struct StorageRecord		*context_storage; /* Storage for pointers in contexts */
	uint						bitmap_size_bits;
	uint						bitmap_size_elements;
	RuntimeBitmapElement		*bitmap_data;

	bool						context_failed; /* Unable to determine full context lists; use fallback mode */

	/* Each time an engine is added or removed, the polymer iteration is increased. If a method is executed whose
	 * iteration is smaller than the iteration counter, it is rechecked.
	 * The iteration counter is initialized to 1, ensuring all methods will be checked initially.
	 */
	uint						polymerIterationChecked;
	bool						polymerMatch;
} methods;
...
typedef struct _jfieldID {
	...
	union
	{
		taint_t	taint;			/* Field taint, for static fields */
		int		taint_index;	/* Index into object's taint array, for object fields */
	} trishul;
	bool	noTaint; 		/* Set if the variable does not hold its taint, only accessible through annotations */
...
// PS
/* Retrieve taint for static field */
#define FIELD_STAINT(FLD)	((FLD)->trishul.taint)
/* Retrieve taint for object field */
#define FIELD_TAINT(OBJ,FLD) (((Hjava_lang_Object *) (OBJ))->member_taint[(FLD)->trishul.taint_index])
#define OBJECT_TNIFIELDS(OBJ)		((Hjava_lang_Object *) (OBJ))->vtable->class->total_instance_field_count
// -PS
