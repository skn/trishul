...
static void dispatchException(Hjava_lang_Throwable*, stackTraceInfo*, taint_t);

static bool findExceptionBlockInMethod(uintp, Hjava_lang_Class*, Method*, uintp*, ContextBasicBlock **);
...
throwException(struct Hjava_lang_Throwable* eobj)
{
	...
	dispatchException(eobj, (stackTraceInfo*)backtrace, 0);
}
 ...
 */
void
throwExternalException(Hjava_lang_Throwable* eobj, taint_t taint)
{
	if (eobj == 0) {
		dprintf("Exception thrown on null object ... aborting\n");
		KAFFEVM_ABORT();
	...
	dispatchException(eobj, (stackTraceInfo*)buildStackTrace(NULL), taint);
}
...
static void
dispatchException(Hjava_lang_Throwable* eobj, stackTraceInfo* baseFrame, taint_t taint)
{
	...
	for (frame = baseFrame; frame->meth != ENDOFSTACK; frame++) {
		bool 				foundHandler;
		uintp 				handler = 0;
		ContextBasicBlock 	*handlerContext = NULL;
		...
		exceptionUnwindStack (eobj, frame->meth, frame->pc, throwMethod, taint, frame->fp, foundHandler);
		throwMethod = frame->meth;
		...
		if (foundHandler) {
			...
			engine_dispatchException(frame->fp, handler, handlerContext, eobj); /* doesn't return */
		}
...
stackOverflowException(struct _exceptionFrame *frame)
{
	...
	dispatchException(soe, (stackTraceInfo*)backtrace, 0);
}
...
nullException(struct _exceptionFrame *frame)
{
	...
	FAKE_THROW_FRAME();
#endif /* defined(HAVE_GCJ_SUPPORT) */
	dispatchException(npe, (stackTraceInfo*)backtrace, 0);
}
...
floatingException(struct _exceptionFrame *frame)
{
	...
	FAKE_THROW_FRAME();
#endif /* defined(HAVE_GCJ_SUPPORT) */
	dispatchException(ae, (stackTraceInfo*)backtrace, 0);
}
 ...
 *
 * handlerContext receives the catch block's first context
 */
static bool
findExceptionBlockInMethod(uintp _pc, Hjava_lang_Class* class, Method* ptr, uintp* handler,
							ContextBasicBlock **handlerContext)
{
