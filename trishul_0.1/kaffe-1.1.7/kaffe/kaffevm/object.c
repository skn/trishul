...
newObjectChecked(Hjava_lang_Class* class, errorInfo *info)
{
	...
	if (!obj) {
		...
		/* PS: Allocate array for member taints */
		if (class->total_instance_field_count)
		{
			assert (class->total_instance_field_count > 0);
			//RecordAllocShadow (class->total_instance_field_count * sizeof (taint_t));
			obj->member_taint = gc_malloc(class->total_instance_field_count * sizeof (taint_t), KGC_ALLOC_NOWALK);
			if (!obj->member_taint)
			{
				postOutOfMemory(info);
			}
...
newClass(void)
{
	...
	/* PS: Allocate array for member taints (a class is an object, too) */
	/* TODO: The number 4 is inferred from classMethod.h. Is there a better way to find this? */
	cls->head.member_taint = gc_malloc(4 * sizeof (taint_t), KGC_ALLOC_CLASSMISC);
	if (!cls->head.member_taint)
	{
		/* PS: TODO: is null allowed here? */
		postOutOfMemory(NULL);
	}

	/* PS: allow processClass to detect if this has been initialized */
	cls->total_instance_field_count = -1;
		...
		if (CLASS_IS_PRIMITIVE(elclass) || elclass == PtrClass) {
			...
			if (((KGC_MAX_MALLOC_TYPE - ARRAY_DATA_OFFSET) / TYPE_SIZE(elclass)) < count)
			...
			else
			  {
				...
				/* PS: Allocate shadow memory for member taints */
				if (count)
				{
					obj->member_taint = gc_malloc (count * sizeof (taint_t), KGC_ALLOC_NOWALK);
					if (!obj->member_taint)
						postOutOfMemory(info);
				}
			  }
