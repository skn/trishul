...
// PS
Hjava_lang_String*
utf8ConstSubString2Java(const Utf8Const *utf8, uint start, uint length)
{
	Hjava_lang_String* string;

	string = utf8ConstSubString2JavaReplace(utf8, start, length, 0, 0);
	unhand(string)->hash = utf8->hash;	/* why not, they're equal */
	return(string);
}
...
// PS
Hjava_lang_String*
utf8ConstSubString2JavaReplace(const Utf8Const *utf8, uint start, uint length, jchar from_ch, jchar to_ch)
{
	const int uniLen = length ? length : (utf8ConstUniLength(utf8) - start);
	Hjava_lang_String* string;
	jchar *jc, buf[200];
	int k;
	...
	/* Convert UTF-8 to Unicode */
	utf8ConstSubStringDecode(utf8, start, uniLen, jc);
...
// PS
/*
 * Return true iff the Utf8Const substring is equal to the Java String.
 */
int
utf8ConstSubStringEqualJavaString(const Utf8Const *utf8, uint utf8_start, uint utf8_length, const Hjava_lang_String *string)
{
	const char *uptr = utf8->data + utf8_start;
	const char *const uend = uptr + (utf8_length ? utf8_length : (strlen(utf8->data) - utf8_start));
	const jchar *sptr = STRING_DATA(string);
	int ch, slen = STRING_SIZE(string);
#if 0
	/* Question: would this optimization be worthwhile? */
