 ...
 */
void
initSecureClasspath(void)
{
	const char* cp;
	classpathEntry* ptr;

	cp = Kaffe_JavaVMArgs.secureClasspath;
	...
	if (cp != NULL && cp[0] != '\0') {
		...
		for (ptr = classpath; ptr != 0; ptr = ptr->next)
		{
			jarFile *jar = openJarFile (ptr->path);
			if (jar == NULL)
			{
				fprintf (stderr, "Unable to load system jar file %s\n", ptr->path);
				exit (1);
			}

			if (!isJarSignatureVerified (jar, TRISHUL_SIGNATURE_ALIAS))
			{
				fprintf (stderr, "System jar file %s is not properly signed\n", ptr->path);
				exit (1);
			}
		}
	}
...
initClasspath(void)
{
	...
	// PS: include command line classpath
	cp = realClassPath;
	if (cp != NULL)
	{
		/* cp may reside in read-only memory, but
