...
slotOffsetNoSpill(SlotData* slot, int type)
{
	...
	// Otherwise, storage does not work
	assert (sizeof (taint_t) == sizeof (int));

	off0 = slot[0].offset;
	if (type == Rlong || type == Rdouble) {
		off1 = slot[0].taint_offset;
...
int
slotOffsetTaint(SlotData* slot, int type, int use UNUSED)
{
SCHK(	sanityCheck();						)
...
SCHK(	sanityCheck();						)
	...
	return (slotOffsetNoSpillTaint(slot, type));
}

int
slotOffsetNoSpillTaint(SlotData* slot, int type)
{
	if (type == Rlong || type == Rdouble) {
		// Otherwise, storage does not work
		assert (sizeof (taint_t) == sizeof (int));
		return slot[1].taint_offset;
	}
	return slot[0].taint_offset;
;
}
/**
 * Invalidates a slot.
