...
#define	SLOT2FRAMEOFFSET_TAINT(_s)				\
	(						\
		((_s) == stack_limit) ?			\
		 STACK_LIMIT_OFFSET() :			\
		((_s) - slotinfo < maxArgs) ? 		\
		 SLOT2ARGOFFSET_TAINT((_s) - slotinfo) :	\
		 SLOT2LOCALOFFSET_TAINT((_s) - slotinfo)	\
	)
...
int	slotOffsetNoSpillTaint(SlotData*, int);
int	slotOffsetTaint(SlotData*, int, int);
...
#define slot_taint(i) seq_slot (s, (i))->taint_offset
