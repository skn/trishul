...
Method *globalMethod;
ContextBasicBlock *currentContext;
...
DBG(MOREJIT,
	...
	// PS: This makes it impossible to retrieve the actual method when a virtual call is made
...
DBG(MOREJIT,
	dprintf ("maxLocal=%d, maxStack=%d, maxTemp=%d, maxArgs=%d\n", maxLocal, maxStack, maxTemp, maxArgs);
	for (i = 0; i <  maxArgs; i++)
	{
		dprintf ("ARG: %d = %d | %d\n", i, SLOT2ARGOFFSET (i), SLOT2ARGOFFSET_TAINT (i));
	}
	for (i = maxArgs; i < maxLocal + maxStack + maxTemp + maxArgs; i++)
	{
		dprintf ("LOCAL: %d = %d | %d\n", i - maxArgs, SLOT2LOCALOFFSET (i), SLOT2LOCALOFFSET_TAINT (i));
	}
	dprintf ("FRAMESIZE: %d, CONTEXTS: %d\n", (SLOTSIZE + sizeof (taint_t)) * (LOCAL_SLOT_COUNT) + CONTEXT_PART_SIZE,
				CONTEXT_PART_SIZE);
);
...
finishInsnSequence(void* dummy UNUSED, nativeCodeInfo* code, errorInfo* einfo)
{
	...
	if (currentContext)
	{
		currentContext->pc_end_orig = currentContext->pc_end;
		currentContext->pc_end = CODEPC - 1;
	}
	...
	// PS: switch order of constant pool/method code so that the jit code header can be determined from
	// a method's code address. This is required for polymorphic invocations
	jch = (jitCodeHeader *)methblock;
	jch->code_start = (void *)(jch + 1);
	jch->code_len = CODEPC;
	jch->pool = ((nativecode *)jch->code_start) + jch->code_len;
 ...
 */
jboolean
initInsnSequence(Method* meth UNUSED, int codesize UNUSED, int localsz, int stacksz, errorInfo* einfo)
{
...
startInsn(sequence* s)
{
	...
	if (currentContext && pc > currentContext->pc_end)
	{
		// Convert pc's to machine code pc's on the fly. Needed for unwinding stack in case of an exception
		currentContext->pc_end_orig = currentContext->pc_end;
		currentContext->pc_end = CODEPC - 1;
		currentContext++;
		currentContext->pc_start_orig = currentContext->pc_start;
		currentContext->pc_start = CODEPC;
		do_rebuild_context_taint (); // TODO: also at first basic block
	}
	// Invalid due to conversion to machine code pc's above
	//assert (!currentContext || (pc >= currentContext->pc_start && pc <= currentContext->pc_end));
}
