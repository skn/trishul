...
spill_int(SlotData* src)
{
	...
	seq_value(&s, 2) = slotOffsetNoSpillTaint(src, Rint);
	HAVE_spill_int(&s);
}
#endif
...
reload_int(SlotData* dst)
{
	...
	seq_value(&s, 2) = slotOffsetNoSpillTaint(dst, Rint);
	HAVE_reload_int(&s);
}
#endif
...
spill_ref(SlotData* src)
{
	...
	seq_value(&s, 2) = slotOffsetNoSpillTaint(src, Rref);
	HAVE_spill_ref(&s);
}
#endif
...
reload_ref(SlotData* dst)
{
	...
	seq_value(&s, 2) = slotOffsetNoSpillTaint(dst, Rref);
	HAVE_reload_ref(&s);
}
#endif
...
spill_long(SlotData* src)
{
	...
	seq_value(&s, 2) = slotOffsetNoSpillTaint(src, Rlong);
	HAVE_spill_long(&s);
}
#endif
...
reload_long(SlotData* dst)
{
	...
	seq_value(&s, 2) = slotOffsetNoSpillTaint(dst, Rlong);
	HAVE_reload_long(&s);
}
#endif
...
spill_float(SlotData* src)
{
	...
	seq_value(&s, 2) = slotOffsetNoSpillTaint(src, Rfloat);
	HAVE_spill_float(&s);
}
#endif
...
reload_float(SlotData* dst)
{
	...
	seq_value(&s, 2) = slotOffsetNoSpillTaint(dst, Rfloat);
	HAVE_reload_float(&s);
}
#endif
...
spill_double(SlotData* src)
{
	...
	seq_value(&s, 2) = slotOffsetNoSpillTaint(src, Rdouble);
	HAVE_spill_double(&s);
}
#endif
...
reload_double(SlotData* dst)
{
	...
	seq_value(&s, 2) = slotOffsetNoSpillTaint(dst, Rdouble);
...
epilogue(Method* meth)
{
	...
	slot_const_const(NULL, (jword)l, (jword) meth, HAVE_epilogue, Tnull);
}
...
mon_enter(methods* meth, SlotInfo* obj)
{
	...
	if ((meth->accflags & ACC_STATIC) != 0) {
		pusharg_class_const(meth->class, 0, true);
		pushtaint_arg_marker ();
		pusharg_class_const(meth->class, 0, false);
	}
	else {
		pusharg_ref(obj, 0, true);
		pushtaint_arg_marker ();
		pusharg_ref(obj, 0, false);
	}
...
#else
	pusharg_ref(mon, 0, true);
	pushtaint_arg_marker ();
	pusharg_ref(mon, 0, false);
	call_soft(lockObject);
	popargs();
#endif
}
...
add_float(SlotInfo* dst, SlotInfo* src, SlotInfo* src2)
{
	...
	pusharg_float(src, 0, true);
	pusharg_float(src2, 1, true);
	pushtaint_arg_marker ();
	pusharg_float(src, 0, false);
	pusharg_float(src2, 1, false);
#else
	pusharg_float(src2, 1, true);
	pusharg_float(src, 0, true);
	pushtaint_arg_marker ();
...
add_double(SlotInfo* dst, SlotInfo* src, SlotInfo* src2)
{
	...
	begin_func_sync();
#if defined(PUSHARG_FORWARDS)
	pusharg_double(src, 0, true);
	pusharg_double(src2, pusharg_long_idx_inc, true);
	pushtaint_arg_marker ();
	pusharg_double(src, 0, false);
	pusharg_double(src2, pusharg_long_idx_inc, false);
#else
	pusharg_double(src2, pusharg_long_idx_inc, true);
	pusharg_double(src, 0, true);
	pushtaint_arg_marker ();
...
_mul_int(SlotInfo* dst, SlotInfo* src, SlotInfo* src2)
{
	...
	begin_func_sync();
#  if defined(PUSHARG_FORWARDS)
	pusharg_int(src, 0, true);
	pusharg_int(src2, 1, true);
	pushtaint_arg_marker ();
	pusharg_int(src, 0, false);
	pusharg_int(src2, 1, false);
#  else
	pusharg_int(src2, 1, true);
	pusharg_int(src, 0, true);
	pushtaint_arg_marker ();
...
mul_long(SlotInfo* dst, SlotInfo* src, SlotInfo* src2)
{
	...
	begin_func_sync();
#if defined(PUSHARG_FORWARDS)
	pusharg_long(src, 0, true);
	pusharg_long(src2, pusharg_long_idx_inc, true);
	pushtaint_arg_marker ();
	pusharg_long(src, 0, false);
	pusharg_long(src2, pusharg_long_idx_inc, false);
#else
	pusharg_long(src2, pusharg_long_idx_inc, true);
	pusharg_long(src, 0, true);
	pushtaint_arg_marker ();
...
#if defined(PUSHARG_FORWARDS)
	pusharg_long(src, 0, true);
	pusharg_int(src2, pusharg_long_idx_inc, true);
	pushtaint_arg_marker ();
	pusharg_long(src, 0, false);
	pusharg_int(src2, pusharg_long_idx_inc, false);
#else
	pusharg_int(src2, pusharg_long_idx_inc, true);
	pusharg_long(src, 0, true);
	pushtaint_arg_marker ();
...
void load_array_length(SlotInfo *dst, SlotInfo *src)
{
	// Allocate a temporary to avoid reusing the array pointer
	SlotInfo* tmpObject;
	slot_alloctmp(tmpObject);
	move_ref (tmpObject, src);

	slot_slot_slot_const_const (dst, src, tmpObject, object_array_length, 0, do_load_array_length, Tload);

	slot_freetmp (tmpObject);
}
...
void
load_int_object_field(SlotInfo* var, SlotInfo* obj, jint offset, Field *field)
{
	// Make sure the object reference lives long enough for taint_array_load_after
	SlotInfo* tmpObject;
	slot_alloctmp(tmpObject);
	move_ref (tmpObject, obj);

	if (offset == 0) {
		taint_object_load_before (var, tmpObject, field);
		load_int(var, tmpObject);
		taint_object_load_after (var, tmpObject, field);
	}
	else
#if defined(HAVE_load_offset_int)
	if (HAVE_load_offset_int_rangecheck(offset)) {
		taint_object_load_before (var, tmpObject, field);
		slot_slot_const(var, tmpObject, offset, HAVE_load_offset_int, Tload);
		taint_object_load_after (var, tmpObject, field);
	}
	else
#endif
	{
		...
		SlotInfo* tmp;
		slot_alloctmp(tmp);
		add_ref_const(tmp, tmpObject, offset);
		load_int(var, tmp);
		slot_freetmp(tmp);
		taint_object_load_after (var, tmpObject, field);
	}
...
void
load_ref_object_field(SlotInfo* var, SlotInfo* obj, jint offset, Field *field)
{
	// Make sure the object reference lives long enough for taint_array_load_after
	SlotInfo* tmpObject;
	slot_alloctmp(tmpObject);
	...
	if (HAVE_load_offset_ref_rangecheck(offset)) {
		taint_object_load_before (var, tmpObject, field);
		slot_slot_const(var, tmpObject, offset, HAVE_load_offset_ref, Tload);
		taint_object_load_after (var, tmpObject, field);
	}
	else
#endif
...
void
load_long_object_field(SlotInfo* var, SlotInfo* obj, jint offset, Field *field)
{
	// Make sure the object reference lives long enough for taint_array_load_after
	SlotInfo* tmpObject;
	slot_alloctmp(tmpObject);
	...
	if (HAVE_load_offset_long_rangecheck(offset)) {
		taint_object_load_before (var, tmpObject, field);
		lslot_slot_const(var, tmpObject, offset, HAVE_load_offset_long, Tload);
		taint_object_load_after (var, tmpObject, field);
	}
	else
#endif
	{
 		taint_object_load_before (var, tmpObject, field);
               /* Don't use LSLOT & HSLOT here */
		load_offset_int(var, tmpObject, offset);
		load_offset_int(var+1, tmpObject, offset+4);
		taint_object_load_after (var, tmpObject, field);
	}
}

void
load_byte_object_field(SlotInfo* var, SlotInfo* obj, jint offset, Field *field)
{
	// Make sure the object reference lives long enough for taint_array_load_after
	SlotInfo* tmpObject;
	slot_alloctmp(tmpObject);
	...
	if (HAVE_load_offset_byte_rangecheck(offset)) {
		taint_object_load_before (var, tmpObject, field);
		slot_slot_const(var, tmpObject, offset, HAVE_load_offset_byte, Tload);
		taint_object_load_after (var, tmpObject, field);
	}
	else
#endif
...
void
load_char_object_field(SlotInfo* var, SlotInfo* obj, jint offset, Field *field)
{
	// Make sure the object reference lives long enough for taint_array_load_after
	SlotInfo* tmpObject;
	slot_alloctmp(tmpObject);
	...
	if (HAVE_load_offset_char_rangecheck(offset)) {
		taint_object_load_before (var, tmpObject, field);
		slot_slot_const(var, tmpObject, offset, HAVE_load_offset_char, Tload);
		taint_object_load_after (var, tmpObject, field);
	}
	else
#endif
...
void
load_short_object_field(SlotInfo* var, SlotInfo* obj, jint offset, Field *field)
{
	// Make sure the object reference lives long enough for taint_array_load_after
	SlotInfo* tmpObject;
	slot_alloctmp(tmpObject);
	...
	if (HAVE_load_offset_short_rangecheck(offset)) {
		taint_object_load_before (var, tmpObject, field);
		slot_slot_const(var, tmpObject, offset, HAVE_load_offset_short, Tload);
		taint_object_load_after (var, tmpObject, field);
	}
	else
#endif
...
void
load_float_object_field(SlotInfo* var, SlotInfo* obj, jint offset, Field *field)
{
	// Make sure the object reference lives long enough for taint_array_load_after
	SlotInfo* tmpObject;
	slot_alloctmp(tmpObject);
	...
	if (HAVE_load_offset_float_rangecheck(offset)) {
		taint_object_load_before (var, tmpObject, field);
		slot_slot_const(var, tmpObject, offset, HAVE_load_offset_float, Tload);
		taint_object_load_after (var, tmpObject, field);
	}
	else
#endif
...
void
load_double_object_field(SlotInfo* var, SlotInfo* obj, jint offset, Field *field)
{
	// Make sure the object reference lives long enough for taint_array_load_after
	SlotInfo* tmpObject;
	slot_alloctmp(tmpObject);
	...
	if (HAVE_load_offset_double_rangecheck(offset)) {
		taint_object_load_before (var, tmpObject, field);
		lslot_slot_const(var, tmpObject, offset, HAVE_load_offset_double, Tload);
		taint_object_load_after (var, tmpObject, field);
	}
	else
#endif
...
void
load_int_array_element(SlotInfo* dst, SlotInfo* src, SlotInfo* idx, int offset)
{
	// Make sure the array reference lives long enough for taint_array_load_after
	SlotInfo* tmpArray;
	slot_alloctmp(tmpArray);
	move_ref (tmpArray, src);
	SlotInfo* tmpIdx;
	slot_alloctmp(tmpIdx);
	move_ref (tmpIdx, idx);

	if (slot_type(tmpIdx) == Tconst) {
		taint_array_load_before (tmpArray, NULL, slot_value(tmpIdx)->i);
		load_offset_int(dst, tmpArray, (jint)(slot_value(tmpIdx)->i * sizeof(jint) + offset));
		taint_array_load_after (tmpArray, dst);
	}
	else
#if defined(HAVE_load_offset_scaled_int)
	{
		taint_array_load_before (tmpArray, tmpIdx, 0);
		slot_slot_slot_const_const(dst, tmpArray, tmpIdx, offset, 0, HAVE_load_offset_scaled_int, Tload);
		taint_array_load_after (tmpArray, dst);
	}
#else
	{
		...
		SlotInfo* tmp;
		slot_alloctmp(tmp);
		lshl_int_const(tmp, tmpIdx, SHIFT_jint);
		add_ref(tmp, tmpArray, tmp);
	...
	slot_freetmp (tmpArray);
	slot_freetmp (tmpIdx);
}

void
load_ref_array_element(SlotInfo* dst, SlotInfo* src, SlotInfo* idx, int offset)
{
	// Make sure the array reference lives long enough for taint_array_load_after
	SlotInfo* tmpArray;
	slot_alloctmp(tmpArray);
	...
	if (slot_type(tmpIdx) == Tconst) {
		taint_array_load_before (tmpArray, NULL, slot_value(tmpIdx)->i);
		load_offset_ref(dst, tmpArray, (jint)(slot_value(tmpIdx)->i * sizeof(jref) + offset));
		taint_array_load_after (tmpArray, dst);
	}
	else
	...
	{
		taint_array_load_before (tmpArray, tmpIdx, 0);
		slot_slot_slot_const_const(dst, tmpArray, tmpIdx, offset, 0, HAVE_load_offset_scaled_ref, Tload);
		taint_array_load_after (tmpArray, dst);
	}
#else
	{
		...
		SlotInfo* tmp;
		slot_alloctmp(tmp);
		lshl_int_const(tmp, tmpIdx, SHIFT_jref);
		add_ref(tmp, tmpArray, tmp);
...
void
load_long_array_element(SlotInfo* dst, SlotInfo* src, SlotInfo* idx, int offset)
{
	// Make sure the array reference lives long enough for taint_array_load_after
	SlotInfo* tmpArray;
	slot_alloctmp(tmpArray);
	...
	if (slot_type(tmpIdx) == Tconst) {
		taint_array_load_before (tmpArray, NULL, slot_value(tmpIdx)->i);
		load_offset_long(dst, tmpArray, (jint)(slot_value(tmpIdx)->i * sizeof(jlong) + offset));
		taint_array_load_after (tmpArray, dst);
	}
	else
	...
	{
		taint_array_load_before (tmpArray, tmpIdx, 0);
		slot_slot_slot_const_const(dst, tmpArray, tmpIdx, offset, 0, HAVE_load_offset_scaled_long, Tload);
		taint_array_load_after (tmpArray, dst);
	}
#else
	{
		...
		SlotInfo* tmp;
		slot_alloctmp(tmp);
		lshl_int_const(tmp, tmpIdx, SHIFT_jlong);
		add_ref(tmp, tmpArray, tmp);
		load_offset_long(dst, tmp, offset);
		slot_freetmp(tmp);

		// When a long is loaded, 2 registers are used without checking the contents. This ensures the
		// array reference is always reloaded.
		// TODO: find a way to do this without useless add operation
		add_ref_const (src, tmpArray, 0);
		taint_array_load_after (src, dst);
	}
#endif
...
void
load_float_array_element(SlotInfo* dst, SlotInfo* src, SlotInfo* idx, int offset)
{
	// Make sure the array reference lives long enough for taint_array_load_after
	SlotInfo* tmpArray;
	slot_alloctmp(tmpArray);
	...
	if (slot_type(tmpIdx) == Tconst) {
		taint_array_load_before (tmpArray, NULL, slot_value(tmpIdx)->i);
		load_offset_float(dst, tmpArray, (jint)(slot_value(tmpIdx)->i * sizeof(jfloat) + offset));
		taint_array_load_after (tmpArray, dst);
	}
	else
	...
	{
		taint_array_load_before (tmpArray, tmpIdx, 0);
		slot_slot_slot_const_const(dst, tmpArray, tmpIdx, offset, 0, HAVE_load_offset_scaled_float, Tload);
		taint_array_load_after (tmpArray, dst);
	}
#else
	{
		...
		SlotInfo* tmp;
		slot_alloctmp(tmp);
		lshl_int_const(tmp, tmpIdx, SHIFT_jfloat);
		add_ref(tmp, tmpArray, tmp);
...
void
load_double_array_element(SlotInfo* dst, SlotInfo* src, SlotInfo* idx, int offset)
{
	// Make sure the array reference lives long enough for taint_array_load_after
	SlotInfo* tmpArray;
	slot_alloctmp(tmpArray);
	...
	if (slot_type(tmpIdx) == Tconst) {
		taint_array_load_before (tmpArray, NULL, slot_value(tmpIdx)->i);
		load_offset_double(dst, tmpArray, (jint)(slot_value(tmpIdx)->i * sizeof(jdouble) + offset));
		taint_array_load_after (tmpArray, dst);
	}
	else
	...
	{
		taint_array_load_before (tmpArray, tmpIdx, 0);
		lslot_slot_slot_const_const(dst, tmpArray, tmpIdx, offset, 0, HAVE_load_offset_scaled_double, Tload);
		taint_array_load_after (tmpArray, dst);
	}
#else
	{
		...
		SlotInfo* tmp;
		slot_alloctmp(tmp);
		lshl_int_const(tmp, tmpIdx, SHIFT_jdouble);
		add_ref(tmp, tmpArray, tmp);
...
void
load_byte_array_element(SlotInfo* dst, SlotInfo* src, SlotInfo* idx, int offset)
{
	// Make sure the array reference lives long enough for taint_array_load_after
	SlotInfo* tmpArray;
	slot_alloctmp(tmpArray);
	...
	{
		taint_array_load_before (tmpArray, NULL, slot_value(tmpIdx)->i);
		load_offset_byte(dst, tmpArray, (jint)(slot_value(tmpIdx)->i * sizeof(jbyte) + offset));
		taint_array_load_after (tmpArray, dst);
	}
	else
	...
	{
		taint_array_load_before (tmpArray, tmpIdx, 0);
		slot_slot_slot_const_const(dst, tmpArray, tmpIdx, offset, 0, HAVE_load_offset_scaled_byte, Tload);
		taint_array_load_after (tmpArray, dst);
	}
#else
	{
...
void
load_char_array_element(SlotInfo* dst, SlotInfo* src, SlotInfo* idx, int offset)
{
	// Make sure the array reference lives long enough for taint_array_load_after
	SlotInfo* tmpArray;
	slot_alloctmp(tmpArray);
	...
	if (slot_type(tmpIdx) == Tconst) {
		taint_array_load_before (tmpArray, NULL, slot_value(tmpIdx)->i);
		load_offset_char(dst, tmpArray, (jint)(slot_value(tmpIdx)->i * sizeof(jchar) + offset));
		taint_array_load_after (tmpArray, dst);
	}
	else
	...
	{
		taint_array_load_before (tmpArray, tmpIdx, 0);
		slot_slot_slot_const_const(dst, tmpArray, tmpIdx, offset, 0, HAVE_load_offset_scaled_char, Tload);
		taint_array_load_after (tmpArray, dst);
	}
#else
	{
		...
		SlotInfo* tmp;
		slot_alloctmp(tmp);
		lshl_int_const(tmp, tmpIdx, SHIFT_jchar);
		add_ref(tmp, tmpArray, tmp);
...
void
load_short_array_element(SlotInfo* dst, SlotInfo* src, SlotInfo* idx, int offset)
{
	// Make sure the array reference lives long enough for taint_array_load_after
	SlotInfo* tmpArray;
	slot_alloctmp(tmpArray);
	...
	if (slot_type(tmpIdx) == Tconst) {
		taint_array_load_before (tmpArray, NULL, slot_value(tmpIdx)->i);
		load_offset_short(dst, tmpArray, (jint)(slot_value(tmpIdx)->i * sizeof(jshort) + offset));
		taint_array_load_after (tmpArray, dst);
	}
	else
	...
	{
		taint_array_load_before (tmpArray, tmpIdx, 0);
		slot_slot_slot_const_const(dst, tmpArray, tmpIdx, offset, 0, HAVE_load_offset_scaled_short, Tload);
		taint_array_load_after (tmpArray, dst);
	}
#else
	{
		...
		SlotInfo* tmp;
		slot_alloctmp(tmp);
		lshl_int_const(tmp, tmpIdx, SHIFT_jshort);
		add_ref(tmp, tmpArray, tmp);
...
void
pusharg_int_const(int val, int idx, bool doTaint)
{
#if defined(HAVE_pusharg_int_const)
	if (HAVE_pusharg_int_const_rangecheck(val)) {
		slot_const_const_const(NULL, val, idx, doTaint, HAVE_pusharg_int_const, Tnull);
		argcount += 1;
	}
	else
#endif
...
void
pusharg_int(SlotInfo* src, int idx, bool doTaint)
{
#if defined(HAVE_pusharg_int_const)
	if (slot_type(src) == Tconst) {
		pusharg_int_const(slot_value(src)->i, idx, doTaint);
	}
	else
#endif
	{
		slot_slot_const_const(NULL, src, idx, doTaint, HAVE_pusharg_int, Tnull);
		argcount += 1;
	}
}
#endif
...
void
pusharg_ref(SlotInfo* src, int idx, bool doTaint)
{
#if defined(HAVE_pusharg_ref_const)
	if (slot_type(src) == Tconst) {
		pusharg_ref_const(slot_value(src)->l, idx, doTaint);
	}
	else
#endif
	{
		slot_slot_const_const(NULL, src, idx, doTaint, HAVE_pusharg_ref, Tnull);
		argcount += 1;
	}
}
#endif
...
void
pusharg_ref_const(void* val, int idx, bool doTaint)
{
#if defined(HAVE_pusharg_ref_const)
	if (HAVE_pusharg_ref_const_rangecheck((jword)val)) {
		slot_const_const_const(NULL, (jword)val, idx, doTaint, HAVE_pusharg_ref_const, Tnull);
		argcount += 1;
	}
	else
#endif
	{
		...
		pusharg_ref(tmp, idx, doTaint);
		slot_freetmp(tmp);
	}
}

void
pusharg_class_const(Hjava_lang_Class* cls, int idx, bool doTaint)
{
#if 1
	pusharg_ref_const(cls, idx, doTaint);
#else
	???
#endif
}
...
void
pusharg_utf8_const(Utf8Const* val, int idx, bool doTaint)
{
#if 1
	pusharg_ref_const(val, idx, doTaint);
#else
	???
#endif
}
...
void
pusharg_float(SlotInfo* src, int idx, bool doTaint)
{
#if defined(HAVE_pusharg_float)
	slot_slot_const_const(NULL, src, idx, doTaint, HAVE_pusharg_float, Tnull);
	argcount += 1;
#elif defined(HAVE_NO_FLOATING_POINT)
	pusharg_int(src, idx, doTaint);
#else
	KAFFEVM_ABORT();
#endif
}
...
void
pusharg_double(SlotInfo* src, int idx, bool doTaint)
{
#if defined(HAVE_pusharg_double)
	lslot_lslot_const_const(NULL, src, idx, doTaint, HAVE_pusharg_double, Tnull);
	argcount += pusharg_long_idx_inc;
#elif defined(HAVE_NO_FLOATING_POINT) || defined(PS2LINUX)
	pusharg_long(src, idx, doTaint);
#else
	KAFFEVM_ABORT();
#endif
}
...
void
pusharg_long(SlotInfo* src, int idx, bool doTaint)
{
#if defined(HAVE_pusharg_long)
	lslot_lslot_const_const(0, src, idx, doTaint, HAVE_pusharg_long, Tnull);
	argcount += pusharg_long_idx_inc;
#elif defined(PUSHARG_FORWARDS)
	/* Don't use LSLOT & HSLOT here */
	pusharg_int(src, idx, doTaint);
	pusharg_int(src+1, idx+1, doTaint);
#else
	/* Don't use LSLOT & HSLOT here */
	pusharg_int(src+1, idx+1, doTaint);
	pusharg_int(src, idx, doTaint);
...
void
call(SlotInfo* dst, const Method *meth)
{
	slot_slot_const_const(NULL, dst, argcount, (jword) meth, HAVE_call, Tnull);
}
#endif
...
call_indirect_method(Method *meth)
{
	...
	if (false && METHOD_TRANSLATED(meth)) {
		_call_soft(METHOD_NATIVECODE(meth), 1);
	}
	else {
		ptr = PMETHOD_NATIVECODE(meth);
#if defined(HAVE_call_indirect_const)
		slot_const_const_const(NULL, (jword)ptr, argcount, (jword) meth, HAVE_call_indirect_const, Tnull);
#else
		{
			SlotInfo* tmp;
			slot_alloctmp(tmp);
...
_call_soft(void *routine, int profiled)
{
	...
	slot_const_const(0, (jword)l, argcount, HAVE_call_soft, Tnull);
	...
	slot_const_const(NULL, (jword)l, argcount, HAVE_call_ref, Tnull);
#else
	label* l;
	constpool* c;
	SlotInfo* tmp;
...
#else
	begin_func_sync();
	pusharg_int(src, NULL, true);
	pushtaint_arg_marker ();
	pusharg_int(src, NULL, false);
...
cvt_float_int(SlotInfo* dst, SlotInfo* src)
{
...
#elif defined(HAVE_cvt_float_int_ieee)
	{
	  ...
	  cbranch_int_const_ne_notaint(tmp, FEXPMASK, reference_label(1, 1));

	  and_int_const(tmp, src, FMANMASK);
	  cbranch_int_const_eq_notaint(tmp, 0, reference_label(1, 2));
...
cvt_double_int(SlotInfo* dst, SlotInfo* src)
{
...
#elif defined(HAVE_cvt_double_int_ieee)
	{
	  ...
	  cbranch_int_const_ne_notaint(LSLOT(tmp), (jint)(DEXPMASK & 0xffffffff), reference_label(1, 1));
	  cbranch_int_const_ne_notaint(HSLOT(tmp), (jint)((DEXPMASK >> 32) & 0xffffffff), reference_label(1, 2));

	  and_long_const(tmp, src, DMANMASK);
	  cbranch_int_const_ne_notaint(LSLOT(tmp), 0, reference_label(1, 3));
	  cbranch_int_const_eq_notaint(HSLOT(tmp), 0, reference_label(1, 4));
...
// First push the argument taints
#if defined(PUSHARG_FORWARDS)
	...
	for (idx = limit-1; idx >= 0; idx--) {
		...
		case 'L':
		case '[':
			pusharg_ref(stack(sp_idx), arg_idx, true);
			break;
		case 'O':
			/* Special: object we've invoking on */
			pusharg_ref(obj, arg_idx, true);
			break;
#if defined(STACK_LIMIT)
		case 'K':
			/* Special: push stack limit */
			pusharg_ref(stack_limit, arg_idx, true);
			break;
#endif
		case 'I':
		case 'Z':
		...
		case 'B':
		case 'C':
			pusharg_int(stack(sp_idx), arg_idx, true);
			break;
		case 'J':
			pusharg_long(stack_long(sp_idx), arg_idx, true);
			break;
		case 'F':
			pusharg_float(stack_float(sp_idx), arg_idx, true);
			break;
		case 'D':
			pusharg_double(stack_double(sp_idx), arg_idx, true);
			break;
		default:
			break;
		}
...
// Now push the arguments
	...
	for (idx = limit-1; idx >= 0; idx--) {
		...
		case 'L':
		case '[':
			pusharg_ref(stack(sp_idx), arg_idx, false);
			break;
		case 'O':
			/* Special: object we've invoking on */
			pusharg_ref(obj, arg_idx, false);
			break;
#if defined(STACK_LIMIT)
		case 'K':
			/* Special: push stack limit */
			pusharg_ref(stack_limit, arg_idx, false);
			break;
#endif
		case 'I':
		case 'Z':
		...
		case 'B':
		case 'C':
			pusharg_int(stack(sp_idx), arg_idx, false);
			break;
		case 'J':
			pusharg_long(stack_long(sp_idx), arg_idx, false);
			break;
		case 'F':
			pusharg_float(stack_float(sp_idx), arg_idx, false);
			break;
		case 'D':
			pusharg_double(stack_double(sp_idx), arg_idx, false);
			break;
		default:
			break;
		}
...
#if defined(PUSHARG_FORWARDS)
	pusharg_ref(obj, 0, true);
	pusharg_utf8_const(meth->name, 1, true);
	pusharg_utf8_const(meth->sig, 2, true);
	pushtaint_arg_marker ();
	pusharg_ref(obj, 0, false);
	pusharg_utf8_const(meth->name, 1, false);
	pusharg_utf8_const(meth->sig, 2, false);
#else
	pusharg_utf8_const(meth->sig, 2, true);
	pusharg_utf8_const(meth->name, 1, true);
	pushtaint_arg_marker ();
...
#if defined(PUSHARG_FORWARDS)
        pusharg_ref(obj, 0, true);
        pusharg_class_const(meth->class, 1, true);
        pusharg_ref_const((void*)(int)meth->idx, 2, true);
		pushtaint_arg_marker ();
        pusharg_ref(obj, 0, false);
        pusharg_class_const(meth->class, 1, false);
        pusharg_ref_const((void*)(int)meth->idx, 2, false);
#else
        pusharg_ref_const((void*)(int)meth->idx, 2, true);
        pusharg_class_const(meth->class, 1, true);
        pusharg_ref(obj, 0, true);
...
check_array_index(SlotInfo* obj, SlotInfo* idx)
{
...
#if defined(HAVE_ccall)
	{
		...
		ccall_soft_int_ugt_notaint(tmp, idx, soft_badarrayindex);
		slot_freetmp(tmp);
	}
#else
#if defined(HAVE_fakecall) || defined(HAVE_fakecall_constpool)
	if (!canCatch(BADARRAYINDEX)) {
		cbranch_offset_int(idx, obj, object_array_length, newFakeCall(soft_badarrayindex, pc), buge | blink | branch_flag_notaint);
	}
	else
#endif
	{
		end_sub_block();
		cbranch_offset_int_ult_notaint(idx, obj, object_array_length, reference_label(1, 1));
...
check_array_store(SlotInfo* array, SlotInfo* obj)
{
	begin_func_sync();
#if defined(PUSHARG_FORWARDS)
	pusharg_ref(array, 0, true);
	pusharg_ref(obj, 1, true);
	pushtaint_arg_marker ();
	pusharg_ref(array, 0, false);
	pusharg_ref(obj, 1, false);
#else
	pusharg_ref(obj, 1, true);
	pusharg_ref(array, 0, true);
	pushtaint_arg_marker ();
...
explicit_check_null(int x, SlotInfo* obj, int y)
{
	...
	{
		end_sub_block();
		cbranch_ref_const_ne_notaint(obj, NULL, reference_label(x, y));
...
check_div(int x, SlotInfo* obj, int y)
{
	...
	{
		end_sub_block();
		cbranch_int_const_ne_notaint(obj, 0, reference_label(x, y));
		softcall_divzero();
		start_sub_block();
		set_label(x, y);
	}
...
check_div_long(int x, SlotInfo* obj, int y)
{
...
#if defined(HAVE_fakecall) || defined(HAVE_fakecall_constpool)
	if (!canCatch(ANY)) {
		cbranch_int_const_notaint(LSLOT(obj), 0, newFakeCall(soft_divzero, pc), eq | blink);
		cbranch_int_const_notaint(HSLOT(obj), 0, newFakeCall(soft_divzero, pc), eq | blink);
	}
	else
#endif
#endif
	{
		end_sub_block();
		cbranch_int_const_ne_notaint(LSLOT(obj), 0, reference_label(x, y+0));
		cbranch_int_const_ne_notaint(HSLOT(obj), 0, reference_label(x, y+1));
		softcall_divzero();
		start_sub_block();
		set_label(x, y+1);
		set_label(x, y+0);
...
{
	begin_func_sync();
	pusharg_utf8_const(name, 0, true);
	pushtaint_arg_marker ();
	pusharg_utf8_const(name, 0, false);
...
softcall_nosuchmethod(struct Hjava_lang_Class* cls, Utf8Const* name, Utf8Const* sig)
{
	begin_func_sync();
#if defined(PUSHARG_FORWARDS)
	pusharg_class_const(cls, 0, true);
	pusharg_utf8_const(name, 1, true);
	pusharg_utf8_const(sig, 2, true);
	pushtaint_arg_marker ();
	pusharg_class_const(cls, 0, false);
	pusharg_utf8_const(name, 1, false);
	pusharg_utf8_const(sig, 2, false);
#else
	pusharg_utf8_const(sig, 2, true);
	pusharg_utf8_const(name, 1, true);
	pusharg_class_const(cls, 0, true);
...
softcall_nosuchfield(Utf8Const* cls, Utf8Const* name)
{
	begin_func_sync();
#if defined(PUSHARG_FORWARDS)
	pusharg_utf8_const(cls, 0, true);
	pusharg_utf8_const(name, 1, true);
	pushtaint_arg_marker ();
	pusharg_utf8_const(cls, 0, false);
	pusharg_utf8_const(name, 1, false);
#else
	pusharg_utf8_const(name, 1, true);
	pusharg_utf8_const(cls, 0, true);
...
{
	begin_func_sync();
	pusharg_class_const(classobj, 0, true);
	pushtaint_arg_marker ();
	pusharg_class_const(classobj, 0, false);
	call_soft(soft_new);
	popargs();
	end_func_sync();
	return_ref(dst);
...
softcall_newarray(SlotInfo* dst, SlotInfo* size, int type)
{
	begin_func_sync();
#if defined(PUSHARG_FORWARDS)
	pusharg_int_const(type, 0, true);
	pusharg_int(size, 1, true);
	pushtaint_arg_marker ();
	pusharg_int_const(type, 0, false);
	pusharg_int(size, 1, false);
#else
	pusharg_int(size, 1, true);
	pusharg_int_const(type, 0, true);
	pushtaint_arg_marker ();
...
softcall_anewarray(SlotInfo* dst, SlotInfo* size, Hjava_lang_Class* type)
{
	begin_func_sync();
#if defined(PUSHARG_FORWARDS)
	pusharg_class_const(type, 0, true);
	pusharg_int(size, 1, true);
	pushtaint_arg_marker ();
	pusharg_class_const(type, 0, false);
	pusharg_int(size, 1, false);
#else
	pusharg_int(size, 1, true);
	pusharg_class_const(type, 0, true);
...
#if defined(PUSHARG_FORWARDS)
	pusharg_class_const(classobj, 0, true);
	pusharg_int_const(size, 1, true);
	for (i = size - 1; i >= 0; i--) {
		pusharg_int(&stktop[i], 1+size-i, true);
	}

	pushtaint_arg_marker ();

	pusharg_class_const(classobj, 0, false);
	pusharg_int_const(size, 1, false);
	for (i = size - 1; i >= 0; i--) {
		pusharg_int(&stktop[i], 1+size-i, false);
	}
#else
	for (i = 0; i < size; i++) {
		pusharg_int(&stktop[i], 1+size-i, true);
...
softcall_checkcast(SlotInfo* objw, SlotInfo* objr, Hjava_lang_Class* class)
{
	...
	begin_func_sync();
#if defined(PUSHARG_FORWARDS)
	pusharg_class_const(class, 0, true);
	pusharg_ref(objr, 1, true);
	pushtaint_arg_marker ();
	pusharg_class_const(class, 0, false);
	pusharg_ref(objr, 1, false);
#else
	pusharg_ref(objr, 1, true);
	pusharg_class_const(class, 0, true);
	pushtaint_arg_marker ();
...
softcall_initialise_class(Hjava_lang_Class* c)
{
	if (c != 0 && c->state != CSTATE_COMPLETE) {
		...
		pusharg_class_const(c, 0, true);
		pushtaint_arg_marker ();
		pusharg_class_const(c, 0, false);
...
softcall_debug1(void* a0, void* a1, void* a2)
{
	begin_func_sync();
#if defined(PUSHARG_FORWARDS)
	pusharg_ref_const(a0, 0, true);
	pusharg_ref_const(a1, 1, true);
	pusharg_ref_const(a2, 2, true);
	pushtaint_arg_marker ();
	pusharg_ref_const(a0, 0, false);
	pusharg_ref_const(a1, 1, false);
	pusharg_ref_const(a2, 2, false);
#else
	pusharg_ref_const(a2, 2, true);
	pusharg_ref_const(a1, 1, true);
	pusharg_ref_const(a0, 0, true);
...
softcall_trace(Method* meth)
{
	...
	begin_func_sync();
#if defined(PUSHARG_FORWARDS)
        pusharg_ref_const(meth, 0, true);
        pusharg_ref(tmp, 1, true);
		pushtaint_arg_marker ();
        pusharg_ref_const(meth, 0, false);
        pusharg_ref(tmp, 1, false);
#else
        pusharg_ref(tmp, 1, true);
        pusharg_ref_const(meth, 0, true);
		pushtaint_arg_marker ();
...
#if defined(PUSHARG_FORWARDS)
	pusharg_ref_const(meth, 0, true);
        pusharg_ref_const(0, 1, true);
	pushtaint_arg_marker ();
	pusharg_ref_const(meth, 0, false);
        pusharg_ref_const(0, 1, false);
#else
        pusharg_ref_const(0, 1, true);
	pusharg_ref_const(meth, 0, true);
	pushtaint_arg_marker ();
...
#if defined(PUSHARG_FORWARDS)
	pusharg_ref(obj, 0, true);
        pusharg_ref_const(meth, 1, true);
	pushtaint_arg_marker ();
	pusharg_ref(obj, 0, false);
        pusharg_ref_const(meth, 1, false);
#else
        pusharg_ref_const(meth, 1, true);
	pusharg_ref(obj, 0, true);
	pushtaint_arg_marker ();
...
softcall_writeref(SlotInfo* from, SlotInfo* to)
{
	...
	begin_func_sync();
#if defined(PUSHARG_FORWARDS)
	pusharg_ref(from, 0, true);
	pusharg_ref(to, 1, true);
	pushtaint_arg_marker ();
	pusharg_ref(from, 0, false);
	pusharg_ref(to, 1, false);
#else
	pusharg_ref(to, 1, true);
	pusharg_ref(from, 0, true);
	pushtaint_arg_marker ();
...
softcall_writeref_static(void* from, SlotInfo* to)
{
	...
	begin_func_sync();
#if defined(PUSHARG_FORWARDS)
	pusharg_ref_const(from, 0, true);
	pusharg_ref(to, 1, true);
	pushtaint_arg_marker ();
	pusharg_ref_const(from, 0, false);
	pusharg_ref(to, 1, false);
#else
	pusharg_ref(to, 1, true);
	pusharg_ref_const(from, 0, true);
...
softcall_writeref(SlotInfo* from, SlotInfo* to)
{
	...
	cbranch_int_const_ne_notaint(tmp1, COLOR_BLACK, reference_label(1, 1));
        slot_freetmp(tmp1);
...
softcall_writeref_static(void* from, SlotInfo* to)
{
	...
	end_sub_block();
	cbranch_int_const_eq_notaint(to, 0, reference_label(1, 2));
...
void taintAdd1IfNotNull (SlotInfo *test, SlotInfo *dst, SlotInfo *src)
{
}

void set_branch_context_1 (SlotInfo *s)
{
}

void taint_static_load (Field *field, SlotInfo *dst)
{
	slot_slot_const (dst, NULL, (jint) field, do_taint_static_load, Tstore);
}

void taint_static_store (Field *field, SlotInfo *src)
{
	slot_slot_const (src, NULL, (jint) field, do_taint_static_store, Tstore);
}

void taint_object_load_before (SlotInfo *dst, SlotInfo *this, Field *field)
{
	slot_slot_const_const (this, NULL, field->trishul.taint_index, 1, do_taint_object_load, Tstore);
}

void taint_object_load_after (SlotInfo *dst, SlotInfo *this, Field *field)
{
	slot_slot_const_const (dst, NULL, field->trishul.taint_index, 0, do_taint_object_load, Tstore);
}

void taint_object_store (SlotInfo *this, Field *field, SlotInfo *src)
{
	slot_slot_const_const(this, src, field->trishul.taint_index,
						  field->signature->data[0], do_taint_object_store, Tstore);
}

void pushtaint_arg_marker ()
{
	slot_slot_const(NULL, NULL, 0, do_pushtaint_arg_marker, Tnull);
}

void taint_array_load_before (SlotInfo* src, SlotInfo* idx, int idxConstant)
{
	slot_slot_const_const (src, idx, idxConstant, 1, do_taint_array_load, Tstore);
}

void taint_array_load_after (SlotInfo *array, SlotInfo* dst)
{
	/* The array slot is used here purely to make sure the register containing the array is kept alive.
	 * This is required as the taint propagation is done after the array load, which might cause the
	 * register to be reused to point to the element rather than the array.
	 */
	slot_slot_const_const (dst, array, 0, 0, do_taint_array_load, Tstore);
}

void taint_array_store (SlotInfo* array, SlotInfo* idx, int offset, SlotInfo* src, char sig)
{
	if (slot_type(idx) == Tconst)
	{
		slot_slot_slot_const_const(array, src, NULL, slot_value(idx)->i, sig, do_taint_array_store, Tstore);
	}
	else
	{
		slot_slot_slot_const_const(array, src, idx, 0, sig, do_taint_array_store, Tstore);
	}
}

void trishul_check_method_annotated_before (Method **methPtr)
{
	// This test can be done here, as annotations are installed before code is generated
	if ((*methPtr)->annotated)
	{
		slot_const_const (NULL, (jword) *methPtr, true, do_store_method_annotation, Tnull);
		*methPtr = (*methPtr)->annotated;
	}
}

void trishul_check_method_annotated_after (Method *orig, Method *annotation)
{
	if (orig != annotation)
	{
		slot_const_const (NULL, (jword) orig, false, do_store_method_annotation, Tnull);
	}
}

void trishul_before_invoke_method (Method *method)
{
	slot_const_const (NULL, true, (jword) method, do_store_nothrow_taint, Tnull);
}

void trishul_after_invoke_method (Method *method)
{
	slot_const_const (NULL, false, (jword) method, do_store_nothrow_taint, Tnull);
}
