...
void
slot_const_const_const(SlotInfo* dst, jword s1, jword s2, jword s3, ifunc f, int type)
{
	sequence* seq = nextSeq();
...
void
slot_slot_const_const(SlotInfo* dst, SlotInfo* s1, jword s2, jword s3, ifunc f, int type)
{
	sequence* seq;
#if defined(TWO_OPERAND)
	/* Two operand systems cannot handle three operand ops.
...
void
lslot_lslot_const_const(SlotInfo* dst, SlotInfo* s1, jword s2, jword s3, ifunc f, int type)
{
	sequence* seq = nextSeq();
