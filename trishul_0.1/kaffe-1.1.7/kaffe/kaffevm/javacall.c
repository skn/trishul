 ...
 */
void
KaffeVM_callMethodA(Method* meth, void* func, void* obj, taint_t objTaint, jvalue* args, jvalue* ret,
		    int promoted)
{
	int i;
	int j;
	...
	for (j = 0; j < METHOD_NARGS(meth); i++, j++) {
		...
		case 'Z':
			...
			call.args[i].taint = args[j].taint; // PS
			break;
		...
		case 'J':
			...
			if (! NO_HOLES) {
				...
				call.args[i+1].taint = args[j].taint; // PS
				i++;
				call.calltype[i] = 0;
				call.callsize[i] = 0;
			}
 ...
 */
void
KaffeVM_callMethodV(Method* meth, void* func, void* obj, taint_t objTaint, va_list args, jvalue* ret)
{
