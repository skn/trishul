...
DBG(MOREJIT,
	dprintf("soft_fixup_trampoline(): meth %p (%s.%s), where %p, native code %p\n",
		meth, meth->class->name->data, meth->name->data, where, PMETHOD_NATIVECODE(meth));
    );
...
void
engine_dispatchException (uintp framePointer, uintp handler, ContextBasicBlock *handlerContext,
			  struct Hjava_lang_Throwable *throwable)
{
  ...
  if (handlerContext->bitmap_index >= 0)
  {
	// Points to the top taint, not the bottom, hence the -bitmap_index
  	taint_t *contextTaintParts = ((taint_t *) framePointer) - 2;
  	contextTaintParts[-handlerContext->bitmap_index] = ((Hjava_lang_Object *) throwable)->obj_taint;
  	//printf ("EXCEPTION: %d %X\n", handlerContext->bitmap_index, ((Hjava_lang_Object *) throwable)->obj_taint);
  }
