...
finishJNIcall(void)
{
	...
	if (eobj != 0) {
		thread_data->exceptObj = NULL;
		throwExternalException(eobj, 0); // PS: TODO
	}
}
...
Kaffe_wrapper(Method* xmeth, void* func, bool use_JNI)
{
	...
	for (j = METHOD_NARGS(xmeth); --j >= 0; ) {
		...
		case 'L':
		case '[':
			pusharg_ref(local(an), count, false);
			break;
		case 'I':
		case 'Z':
		case 'S':
		case 'B':
		case 'C':
			pusharg_int(local(an), count, false);
			break;
		case 'F':
			pusharg_float(local(an), count, false);
			break;
		case 'J':
			count--;
			an--;
			pusharg_long(local(an), count, false);
			break;
		case 'D':
			count--;
			an--;
			pusharg_double(local(an), count, false);
			break;
		default:
			break;
		}
	...
	if (use_JNI) {
		/* If static, push the class, else push the object */
		if (METHOD_IS_STATIC(xmeth)) {
			pusharg_ref_const(xmeth->class, 1, false);
		}
		else {
			pusharg_ref(local(0), 1, false);
		}

		/* Push the JNI info */
		pusharg_ref(tmp, 0, false);
	}
	else {
		/* If static, nothing, else push the object */
		if (!METHOD_IS_STATIC(xmeth)) {
			pusharg_ref(local(0), 0, false);
		}
	}
...
#error "Unsupported size of pointer"
			...
			end_sub_block();
			begin_sync();
			cbranch_int_const_eq_notaint(tmp2, 0, reference_label(1, 1));
			end_sync();
